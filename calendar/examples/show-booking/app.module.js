import { ResourceModel, EventModel, Calendar, NumberFormat, StringHelper, DateHelper, ArrayHelper, Popup } from '../../build/calendar.module.js';
...
const calendar = new Calendar({
    ...
    showTipRenderer({ eventRecord : performance }) {
        return {
            children : [
                {
    ...
    onPerformanceAdded({ eventRecord }) {
    ...
    findAlternateShows({ date, domEvent }) {
        const
            performances = this.eventStore.getEvents({
                startDate     : date,
    ...
    performanceRenderer({ eventRecord, renderData }) {
    ...
    onAdminToggle({ source, value }) {
        const
            me               = this,
            { showSelector } = me.widgetMap;
        ...
        if (value) {
            ...
            me.onShowSelect({ value : me.widgetMap.showSelector.value });
    ...
    bookShow({ eventRecord : performance }) {
    ...
    onSeatSelectionChange({ source }) {
        const { widgetMap } = this.bookingPopup;
