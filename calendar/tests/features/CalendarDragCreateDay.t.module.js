...
StartTest(t => {
    ...
    t.describe('All Day', t => {
        ...
        t.it('Should use configured dragUnit', async t => {
            await harness.init(t, {
                modes : {
                    day : {
                        allDayEvents : {
            ...
            // We overrode the default for CalendarRow which is 'day'
            t.is(harness.newEventRecord.durationUnit, 'hour');
            t.is(harness.newEventRecord.duration, 24);
        });
    });
    ...
    t.describe('Day Detail', t => {
        ...
        t.it('Should show a tooltip if configured to do so with object', async t => {
            ...
            // Turbo mouse-moving doesn't give time for the tooltip to show, so must be two stages.
            await t.dragBy('.b-dayview-day-detail.b-calendar-cell', [0, 10], null, null, null, true);
            await t.moveCursorBy([0, 40]);

            await t.waitForSelector('.b-tooltip:contains(New event)');
        });
        ...
        t.it('Should use configured dragUnit', async t => {
            ...
            // Drag-create a 3 hour event
            await harness.dayTimeDrag({
                from : { date : '2019-10-14', time : '2:00' },
                to   : { date : '2019-10-14', time : '5:00' }
            });

            // We overrode the default for DayView which is 'hour'
            t.is(harness.newEventRecord.durationUnit, 'second');
            t.is(harness.newEventRecord.duration, 3 * 60 * 60);
        });
    });
