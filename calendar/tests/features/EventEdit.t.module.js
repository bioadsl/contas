...
StartTest(t => {
    ...
    t.it('Should be able to change the name and date', async t => {
        ...
        let eventEditor, calendarField, pickerItem0, cal0Color, newEventIcon;
        ...
        t.chain(
            ...
            {
                waitFor : () => {
                    newEventIcon = t.query('[data-date="2019-10-23"] .b-cal-event-icon')[0];

                    return DomHelper.getStyleValue(newEventIcon, 'color') === cal0Color;
                }
            },
            ...
            () => {
                ...
                t.is(DomHelper.getStyleValue(newEventIcon, 'color'), cal0Color, 'Event has correct colour');
            }
        );
    });
    ...
    // https://github.com/bryntum/support/issues/2416
    t.it('Should clear isCreating flag when vetoing an edit of a new event', async t => {
        calendar = await getCalendar({
            events    : t.getHackathonData().events.rows,
            date      : new Date(2019, 9, 14),
            listeners : {
        ...
        await calendar.editEvent(event);

        t.selectorNotExists('.b-eventeditor', 'No editor');

        t.is(event.isCreating, false);
    });
    ...
    t.it('Should open all combos, datefields and timefields in all themes and all locales', async t => {
        ...
        await t.forEveryThemeAndLocale(async() => {
            ...
            for (const trigger of t.query(`.b-pickerfield:not(.b-timefield):visible`)) {
            ...
            calendar.features.eventEdit.editor.close();
        });
    });

    // https://github.com/bryntum/support/issues/6973
    t.it('Should support editing a new EventModel instance without values', async t => {
        calendar = await t.getCalendar({
            features : {
                eventEdit : true
            },
        ...
        await calendar.editEvent(record);

        await t.waitForSelector('.b-eventeditor');
    });

    // https://github.com/bryntum/support/issues/7085
    t.it('Should allow prevent saving and deleting in beforeEventSave and beforeEventDelete', async t => {
        let beforeEventSaveCalled, beforeEventDeleteCalled;
        ...
        calendar = await t.getCalendar({
            ...
            listeners : {
                beforeEventSave() {
                    beforeEventSaveCalled = true;
                    return false;
                },
                beforeEventDelete() {
                    beforeEventDeleteCalled = true;
                    return false;
                }
            }
        });

        t.wontFire(calendar.eventStore, 'update');
        t.wontFire(calendar.eventStore, 'remove');

        await t.doubleClick('.b-cal-event');
        await t.type('input[name="name"]', 'Foo bar');
        await t.click('button[data-ref="saveButton"]');
        await t.waitFor(() => beforeEventSaveCalled);
        t.is(calendar.eventStore.first.name, '', 'Prevented saving');

        await t.click('button[data-ref="deleteButton"]');
        await t.waitFor(() => beforeEventDeleteCalled);
        t.ok(calendar.eventStore.first, 'Prevented deleting');
    });

    t.it('Should allow prevent saving and deleting in beforeEventSave and beforeEventDelete with async handlers', async t => {
        let beforeEventSaveCalled, beforeEventDeleteCalled;
    ...
    t.it('Should allow preventing recurring event popup in beforeEventSave and beforeEventDelete', async t => {
        let beforeEventSaveCalled, beforeEventDeleteCalled;
        ...
        // wait for some timeout to make sure recurrence popup won't show up
        await t.waitFor(200);
        await t.waitForSelectorNotFound('.b-recurrenceconfirmationpopup');
    ...
    t.it('Should allow preventing recurring event popup in beforeEventSave and beforeEventDelete with async handler', async t => {
        let beforeEventSaveCalled, beforeEventDeleteCalled;
        ...
        await t.click('button[data-ref="deleteButton"]');
        await t.waitFor(() => beforeEventDeleteCalled);
        t.is(calendar.eventStore.first.name, '', 'Prevented deleting');
    ...
    // https://github.com/bryntum/support/issues/7196
    t.it('Should apply autoCreate step to timeField step', async t => {
        calendar = await t.getCalendar({
            events : [{
                id             : 'test-event',
                name           : '',
        ...
        await t.doubleClick('.b-cal-event');
        await t.waitForSelector('.b-eventeditor');
        t.is(t.query('.b-eventeditor [data-ref="startTimeField"] input')[0].value, '9:00 AM', 'Correct time before increment');
        await t.click('[data-ref="startTimeField"] [data-ref="forward"]');
        t.is(t.query('.b-eventeditor [data-ref="startTimeField"] input')[0].value, '9:40 AM', 'Correct time after increment');
    });

    // https://github.com/bryntum/support/issues/7224
    t.it('Should remove event from list when it is in isCreating:true state', async t => {
        let newEvent = null;
        ...
        await t.waitFor(() => newEvent);
        newEvent.remove();

        t.is(calendar.eventStore.count, 0);
        t.is(calendar.activeView.store.count, 0);
    });
});
