Class('BryntumCalendarTest', {
    ...
    override : {
        ...
        checkCalendarSanity(widget) {
            const cells = widget.element.querySelectorAll('[data-date]:not(.b-group-row)');
        ...
        setupDragHarness({ activeIndex, config, viewSelector, cellCls, innerSelector, overflowSuffix }) {
            ...
            const harness = {
                ...
                async init(_t, localConfig = {}) {
                    ...
                    // Check that the creation view's autoCreate preferences were used.
                    harness.calendar.on({
                        beforeDragCreateEnd({ view, eventRecord }) {
                            // CalendarDrag will use the view's setting if preset
                            const durationUnit = view.dragUnit || harness.calendar.features.drag.durationUnit;

                            // The durationUnit in the drag-created record must use the same units
                            // as specified in the dragUnit of the view.
                            t.is(eventRecord.durationUnit, durationUnit);
                        }
                    });
            ...
            Object.defineProperty(harness, 'WEEK_PX_PER_DAY', {
                get : () => harness.calendar.activeView.dayContainerElement.querySelector('[data-date]').offsetWidth
            });
