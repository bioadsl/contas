    ...
    methods : {
        ...
        waitForAnimations(callback = () => {}) {
            return this.SUPER(() => {
                ...
                if (bryntum && bryntum.testProject && !bryntum.testProject.isDestroyed) {
                    return this.waitForProjectReady(bryntum.testProject, callback);
                }
                else {
                    callback();
                }
        ...
        async assertColumnLines(skipExists = false) {
            const
                bottomCells = this.query(`.b-sch-header-row-1 .b-sch-header-timeaxis-cell`),
                topCells    = this.query(`.b-sch-header-row-0 .b-sch-header-timeaxis-cell`);

            if (bottomCells.length > 1) {
                await this.waitForSelector('.b-column-line');
                this.pass('Column lines found');
                ...
                bottomCells.forEach((element, i) => {
                    if (i) {
                        ...
                        if (line && Math.abs(line.getBoundingClientRect().left - left) > 0.5) {
                            this.fail(`Misalign for bottom line ${element.dataset.tickIndex}, expected ${left}, got ${line.offsetLeft}`);
                            bottomCorrect = false;
                        }
                    }
                });
                ...
                if (bottomCorrect) {
                    this.pass('Bottom lines align');
                }
            }

            if (topCells.length > 1) {
                if (!skipExists) {
                    await this.waitForSelector('.b-column-line-major');
                    this.pass('Major lines found');
                }
                ...
                topCells.forEach((element, i) => {
                    if (i) {
                        ...
                        // Some lines might be replaced by major lines, thus the check that line exists
                        if (line && Math.abs(line.getBoundingClientRect().left - left) > 0.5) {
                            this.fail(`Misalign for major line ${element.dataset.tickIndex}, expected ${left}, got ${line.offsetLeft}`);
                            topCorrect = false;
                        }
                    }
                });
