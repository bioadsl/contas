...
StartTest(t => {
    ...
    // https://github.com/bryntum/support/issues/6705
    t.it('Date header click', async t => {
        calendar = await t.getCalendar({
            date   : new Date(2021, 2, 27),
            events : [{
                name      : 'Test event',
                startDate : '2021-03-28T00:00',
                endDate   : '2021-03-28T03:00',
                id        : 0
            }],
            modes : {
                day : {
        ...
        await t.doubleClick('.b-day-name-date');

        await t.waitFor(() => calendar.features.eventEdit.editor?.containsFocus);

        await t.type(null, 'New All day[ENTER]');

        await t.click('.b-cal-event-wrap[data-event-id="0"]');

        await t.waitForSelector('.b-sch-event-tooltip:contains(Test event)');

        // The event is after midnight, but must *not* change the Calendar date.
        // The event is still logically part of March 27th
        await t.click('.b-sch-event-tooltip:contains(Test event) [data-ref="close"]');

        t.is(calendar.date, new Date(2021, 2, 27));
    });
});
